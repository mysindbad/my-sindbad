...
export async function getNearbyPlacesContext(lat, lon){
  ...
  for (var i = 0; i < els.length; i++) {
    ...
    if (!name) continue;
    var lat2 = el.lat != null ? el.lat : (el.center && el.center.lat);
    var lon2 = el.lon != null ? el.lon : (el.center && el.center.lon);
    if (!Number.isFinite(lat2) || !Number.isFinite(lon2)) continue;
    out.push({ name: name, category: (el.tags && (el.tags.tourism || el.tags.amenity)) || 'place', lat: lat2, lon: lon2 });
...
export function placesContextText(places){
  ...
  return 'أماكن حقيقية قريبة من الوجهة (من OpenStreetMap) يمكنك الاختيار منها:' + NL + places.map(function (p){ return '- ' + p.name + ' (' + p.category + ')'; }).join(NL);
}
