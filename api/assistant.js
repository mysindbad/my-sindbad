...
export default async function handler(req, res) {
  ...
  // Phase 15 — Inject validated user location as ephemeral context only. No storage, no history, no extra API calls.
  if (location) {
    const nl = String.fromCharCode(10);
    const locLine = 'موقع المستخدم الحالي (تقريبي، مؤقت): ' + location.lat.toFixed(4) + ', ' + location.lng.toFixed(4) + (location.accuracy != null ? ' (دقة ~' + Math.round(location.accuracy) + 'م)' : '') + '. استخدمه لتقديم توصيات قريبة من موقع المستخدم فقط عندما يكون السؤال عن أماكن قريبة أو أنشطة الآن. لا تخزنه ولا تضعه في الـhistory.';
    contextBlock = contextBlock ? contextBlock + nl + nl + locLine : 'سياق إضافي للذكاء:' + nl + locLine;
  }
